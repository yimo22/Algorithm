...
bool move() {
    for (Node *cur : nodelist) {
        ...
        // red && white
            ...
            vector<Node *> insertList;
            int idx = 0;
            for (Node *node : counting[cur->x][cur->y]) {
                if (node == cur) break;
                idx++;
            }
            // making insert list
            insertList.insert(insertList.end(),
                              counting[cur->x][cur->y].begin() + idx,
                              counting[cur->x][cur->y].end());
            // erase list
            counting[cur->x][cur->y].erase(
                counting[cur->x][cur->y].begin() + idx,
                counting[cur->x][cur->y].end());
